...
> **当前项目状态更新**：本项目已全面采用 **Vision OCR 视觉识别方案（L1-L5 模块化架构）**（无需关闭 SIP、无需数据库解密）。下文中的"数据库解密方案"仅作为技术调研背景保留，实际实现请参考 `wechat_rpa/bot/wechat_bot.py` 和 `ARCHITECTURE.md`。
...
```
wechat-mac-rpa/
├── ✅ wechat_rpa/bot/wechat_bot.py         # Vision OCR 全自动机器人（当前唯一版本）
...
**需要全自动机器人**
→ 使用 `wechat_rpa/bot/wechat_bot.py`（当前唯一维护版本）
