...
- **文件**: `wechat_rpa/utils/llm_client.py:13`
- **代码**:
  ```python
...
### C2. run_bot.py 从顶层 utils 导入而非 wechat_rpa.utils
- **文件**: `run_bot.py:7`
- **代码**: `from utils.qwen_client import QwenClient`
- **问题**: 项目同时存在两个 `utils/` 目录：项目根目录的 `utils/` 和 `wechat_rpa/utils/` 。run_bot.py 使用了顶层的 `utils/qwen_client.py`，但 `wechat_rpa/utils/llm_client.py` 中定义了功能重叠的 KimiClient。
- **影响**: 代码分裂、维护困难。如果将来删除顶层 `utils/`，run_bot.py 直接崩溃。
- **修复**: 统一移入 `wechat_rpa/utils/`，修复所有导入路径。

### C3. 80+ 处广泛异常吞噬
- **重灾区**:
  - `wechat_rpa/bot/wechat_bot.py` — 6 处
  - `wechat_rpa/perception/smart_pipeline.py` — 5 处
  - `wechat_rpa/reply/generator.py` — 3 处
  - `wechat_rpa/memory/engine.py` — 6 处
  - `wechat_rpa/storage/chat_history.py` — 4 处
  - `wechat_rpa/storage/message_store.py` — 2 处
  - `wechat_rpa/session/global_store.py` — 2 处
  - `wechat_rpa/llm/openclaw_client.py` — 1 处
  - `wechat_rpa/ocr/vision_ocr.py` — 1 处
  - `其他脚本和测试文件` — 50+ 处
...
- **文件**:
  ...
  - `wechat_rpa/storage/message_store.py`
...
- **文件**: `wechat_rpa/perception/smart_pipeline.py` (SmartScreenCache 类)
...
- **文件**: `wechat_rpa/perception/smart_pipeline.py`
...
- **文件**: `wechat_rpa/bot/wechat_bot.py` (多处调用)
- **问题**:
...
- **文件**: `wechat_rpa/llm/openclaw_client.py`
...
- **文件**: `wechat_rpa/bot/wechat_bot.py:361`
- **代码**:
  ```python
  finally:
...
- **文件**: `wechat_rpa/session/global_store.py`, `wechat_rpa/perception/smart_pipeline.py`
...
- **文件**: `wechat_rpa/logging/bot_logger.py:124-125`
...
- **文件**: `wechat_rpa/perception/vision_pipeline.py:96`
...
- **文件**: `utils/qwen_client.py`, `wechat_rpa/utils/llm_client.py`, `wechat_rpa/llm/openclaw_client.py`
- **行为对比**:
  | Client | 出错时 | 返回值 |
  |--------|--------|--------|
...
- **文件**: `wechat_rpa/reply/generator.py:170`
...
### M4. 测试文件分散在两个目录
- **结构**: `tests/` (8 个) + `wechat_rpa/tests/` (17 个)
...
| L3 | `utils/qwen_client.py` 与 `wechat_rpa/utils/llm_client.py` 功能重叠但接口不同 | `utils/*.py` |
| L4 | `wechat_rpa/llm/openclaw_client.py` 中 OpenClawClient 与 KimiClient 职责重叠 | `llm/*.py` |
...
**位置**: `wechat_rpa/memory/engine.py:277`
**代码**:
```python
...
**位置**: `wechat_rpa/ocr/vision_ocr.py:52-71`
...
**位置**: `wechat_rpa/action/message_sender.py:58`
...
**位置**: `wechat_rpa/capture/window_capture.py:184-187`
**代码**:
```python
...
**位置**: `wechat_rpa/memory/engine.py:444`
...
**位置**: `wechat_rpa/memory/engine.py:430-439`
**代码**:
```python
...
**位置**: `wechat_rpa/storage/chat_history.py:156`
...
**位置**: `wechat_rpa/layout/layout_parser.py:59`
**代码**:
```python
...
**位置**: `wechat_rpa/capture/window_capture.py:147-160`, `wechat_rpa/action/login_recovery.py:78-86`
...
**位置**: `wechat_rpa/perception/smart_pipeline.py`
...
**位置**: `wechat_rpa/action/ui_interactor.py:46-48`
**代码**:
```python
...
**位置**: `wechat_rpa/ocr/vision_ocr.py:148-154`
**代码**:
```python
@property
...
**位置**: `wechat_rpa/tools/builtin_tools.py`
...
**位置**: `wechat_rpa/layout/layout_parser.py:59, 258`, `wechat_rpa/message/extractor.py`可能存在
...
**位置**: `wechat_rpa/perception/smart_pipeline.py` 多处 docstring
...
**位置**: `wechat_rpa/perception/vision_pipeline.py:64`
**代码**:
```python
...
**位置**: `wechat_rpa/capture/window_capture.py:84`
