...
例：输入 `"王芊 同事"` → `raw_keywords = ["王芊", "同事"]` → `keywords = ["王芊", "同事"]`（假设王芊没有别名需要扩展）
...
**陷阱**: `substring_match` 用的是 `resolved_keyword in name`，不是 `name in resolved_keyword`。所以搜 "王芊" 时：
- ✅ "王芊.md" → primary（name_match）
- ✅ "王芊_2.md" → primary（substring_match: "王芊" in "王芊_2"）
- ✅ "W1han、秋水文章、王芊.md" → primary（substring_match: "王芊" in "W1han、秋水文章、王芊"）
- ❌ "王.md" → **不是 primary**（"王芊" not in "王"）
...
例：文档里 "同事" 出现 30 次，"王芊" 出现 1 次。如果 "同事" 在 keywords 列表中排在前面，可能提取的 2 个片段都围绕 "同事"，完全不包含 "王芊"。
...
| 问题 | 现象 | 根因 |
|------|------|------|
| Primary 过多挤占 Top 5 | 搜 "王芊 同事" 只返回王芊自己的 wiki，程立/Brian 被挤出 | 4 个文件名含 "王芊" 的文档都被判为 primary，占了 Top 4 |
