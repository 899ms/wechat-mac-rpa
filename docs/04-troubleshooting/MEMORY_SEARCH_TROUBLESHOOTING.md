...
| 日志 | 含义 | 正常值示例 |
|------|------|-----------|
| `keyword='...' raw=[...] expanded=[...] resolved='...'` | 搜索词及扩展后的关键词 | `keyword='王芊 同事' raw=['王芊','同事'] expanded=['王芊','干哥',...,'同事']` |
...
3. **相关人物的 wiki 里不包含搜索关键词**：比如搜"王芊 同事"，但程立的 wiki 里写的是"一起工作"而不是"同事"
...
grep -l "王芊\|同事" data/memory/wiki/users/*.md | head -10
...
# 检查噪声文档的内容
grep -n "王芊\|同事" data/memory/wiki/users/噪声文档.md
```
...
2. **搜索词设计问题**：搜"王芊 同事"只能召回包含这两个词的文档，无法召回只写"与王芊共事"的文档
3. **跨文档关联缺失**：BM25 是单文档匹配，不支持跨文档 join

**快速验证**：
```bash
# 检查哪些人的 wiki 里写了关系
grep -l "王芊.*同事\|同事.*王芊" data/memory/wiki/users/*.md

# 检查本人的 wiki 里有没有同事列表
grep -n "同事" data/memory/wiki/users/王芊.md
```
...
```python
resolved = engine._resolve_alias("王芊")
...
```python
content = open("data/memory/wiki/users/程立-君奕.md").read()
keywords = ["王芊", "同事"]
