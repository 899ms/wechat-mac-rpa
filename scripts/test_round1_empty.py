...
def test_round1_with_tool_result():
    ...
    messages = [
        ...
        {"role": "assistant", "content": "", "tool_calls": [
            {"id": "call_abc123", "type": "function", "function": {"name": "search_memory", "arguments": '{"query": "周蓓蓓"}'}}
        ]},
        {"role": "tool", "tool_call_id": "call_abc123", "content": "周蓓蓓是王芊的朋友，2023年一起旅游认识的。"},
    ]
    resp = client.chat.completions.create(
        model=MODEL, messages=messages, tools=tools, max_tokens=2000, timeout=30
    )
