...
from wechat_rpa.models.base import ChatListItem, OCRTextElement, Rect
        ...
        for uc in unread_candidates:
            ...
            for idx, anchor_y in enumerate(group_anchor_y):
                ...
                if dist < best_dist and dist < 60:
                    ...
                    best_idx = idx
            if best_idx >= 0:
                unread_for_group[best_idx] = uc.text
        ...
        if image_path:
            try:
                ...
                for idx, anchor_y in enumerate(group_anchor_y):
                    ...
                    avatar_top = nick_top - 15
                    # 头像在昵称左侧，宽度约 50-55px
                    nick_left = groups[idx][0].bbox.x
                    avatar_left = max(0, nick_left - 55)
                    
                    # badge 在头像左上角，取头像左上 25x25 区域
                    y1 = max(0, avatar_top)
                    y2 = min(img_arr.shape[0], avatar_top + 25)
                    x1 = avatar_left
                    x2 = min(img_arr.shape[1], avatar_left + 25)
