...
4. 消息 type 分类（重要新增，最容易出错）：
   - "text"：纯文字消息。包含 emoji 符号（如😂🧐🤝）的文字消息也属于 text，不是 sticker
   - "image"：图片、照片、截图等非表情类的图像内容
   - "sticker"：独立的表情包文件（微信表情商店下载的大表情、GIF 动图、熊猫头等第三方表情包）
   - "mixed"：图文混排（消息同时包含图片和文字）
   - "link_card"：链接卡片、文章分享、小程序卡片
   - 【绝对规则】白色/灰色气泡里的内容，只要不是独立的图片/表情包文件，一律识别为 text
   - 【常见错误】不要把带 emoji 的文字消息错标为 sticker。例如"哈哈😂"是 text，不是 sticker
   - 【常见错误】不要把对方发送的纯文字消息（即使很短）错标为 image/sticker
   - 判断标准：看消息气泡里的主要内容。如果气泡内主要是图像 → image/sticker；如果主要是文字（含 emoji）→ text
