...
class KimiClient:
    ...
    def chat(self, messages=None, user_id=None, message=None, system_prompt=None) -> str:
        """生成回复
        
        支持两种调用方式：
        1. 新接口（兼容 ReplyGenerator）: chat(messages=[{"role": "system", ...}, {"role": "user", ...}])
        2. 旧接口（带会话历史）: chat(user_id="xxx", message="...", system_prompt="...")
        """
        if messages is not None:
            return self._chat_with_messages(messages)
        return self._chat_with_user_id(user_id, message, system_prompt)

    def _chat_with_messages(self, messages: List[dict]) -> str:
        """直接透传 messages 列表调用大模型（无会话历史维护）"""
        try:
