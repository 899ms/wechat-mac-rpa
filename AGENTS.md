...
**不要在消费端加保护代码来"防住"上游的脏数据或异常。让问题暴露出来，才能被定位和修复。**

典型案例：
- ❌ `if "（暂无）" not in wiki:` —— 整段 wiki 被过滤，用户永远不知道群 wiki 没显示
- ❌ `if len(text) < 5: return ""` —— 短消息被静默丢弃，没人知道丢了消息
- ❌ `try: ... except: return ""` —— 异常被吞掉，返回空字符串假装"一切正常"

**正确做法只有一个：不要保护。**
- 上游返回什么就展示什么，即使结果是"（暂无）"、乱码或空字符串
- 用户看到丑数据会反馈 → 推动修复上游根因
- 如果确实需要容错（如避免 crash），必须打 `logger.warning` 留下痕迹

**核心原则：宁可在输出里看到丑数据，也不要用隐藏逻辑让问题永远不被发现。**
